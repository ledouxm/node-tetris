...
import { getCumulativeHeight, getHoles, getRoughness } from "./pomped";
...
export const getAggregatedHeight = (basePeaks: number[]) =>
  basePeaks.reduce(
    (acc, current) => (current === HEIGHT ? acc : acc + HEIGHT - 1 - current),
    0
  );
...
export const getPeaks = (grid: Grid) => {
  ...
  while (peaks.length < WIDTH) {
    ...
    if (currentHeight === HEIGHT) {
      ...
      ![EMPTY_CELL, CURRENT_CELL].includes(grid[currentHeight][peaks.length])
    ) {
...
export const getMinMaxHeight = (grid: Grid) => {
  ...
  const max = peaks.reduce(
    (acc, current) =>
      current === HEIGHT ? acc : Math.max(acc, HEIGHT - 1 - current),
    0
  );
  let min = HEIGHT;
  let y = 0;
...
export const getAverageHeight = (grid: Grid) => {
  ...
  return (
    peaks.reduce((avg, current) => avg + (HEIGHT - current), 0) / peaks.length
  );
};
